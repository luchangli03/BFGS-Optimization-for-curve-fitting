...
// data type, ParaNum, ItNum, ItNum_bs; 
template <class IType, int DataArrayNum, int ParaNum, int IterateNum, int IterateNum_bs> 
class BFGSOptimizer_TypeDef
{
public:
...
private :
	...
	void(*pPreFitting)(float *FitPara, IType *DataArrayList[DataArrayNum], int DataNum);
	float(*pTargerF)(float *FitPara, IType *DataArrayList[DataArrayNum], int DataNum);
...
public:
	...
	BFGSOptimizer_TypeDef(void(*ipPreFitting)(float *FitPara, IType *DataArrayList[DataArrayNum], int DataNum), float(*ipTargerF)(float *FitPara, IType *DataArrayList[DataArrayNum], int DataNum))
	{
		int cnt;
	...
	void BFGSOptimize(IType *DataArrayList[DataArrayNum], int DataNum)
	{
		int rcnt;
		// initial guess
		...
		if (pPreFitting != NULL)
		{
			PreFitting(FitPara, DataArrayList, DataNum);
		...
		GradientCalc(FitPara, grad, DataArrayList, DataNum);
		...
		BFGSOptimizer_Core(DataArrayList, DataNum);
...
private:
	...
	void BFGSOptimizer_Core(IType *DataArrayList[DataArrayNum], int DataNum)
	{
		// adjust d0
		float td0_total;
		...
		for (itcnt = 0; itcnt<IterateNum; itcnt++)
		{
			...
			ddat[0] = TargerFunction(&xd[0], DataArrayList, DataNum);
			ddat[1] = TargerFunction(&xd[ParaNum], DataArrayList, DataNum);
			...
			for (bcnt = 0; bcnt < IterateNum_bs; bcnt++)
			{
				...
				if (bcnt < IterateNum_bs - 1)
				{
					...
					ddat[xdsel] = TargerFunction(&xd[xdsel*ParaNum], DataArrayList, DataNum);
				}
	...
	void PreFitting(float *FitPara, IType *DataArrayList[DataArrayNum], int DataNum)
	{
		pPreFitting(FitPara, DataArrayList, DataNum);

	}

	float TargerFunction(float *FitPara, IType *DataArrayList[DataArrayNum], int DataNum)
	{
		...
		return pTargerF(tFitPara, DataArrayList, DataNum);
	}

	void GradientCalc(float *FitPara, float *grad, IType *DataArrayList[DataArrayNum], int DataNum)
	{
		float tx0[ParaNum];
		float tgradn;
		float tgradp;
		...
		for (cnt = 0; cnt < ParaNum; cnt++)
		{
			...
			tgradn = TargerFunction(tx0, DataArrayList, DataNum);

			tx0[cnt] = tx0[cnt] + 0.004f;
			tgradp = TargerFunction(tx0, DataArrayList, DataNum);
